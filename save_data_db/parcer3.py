...
DB_PATH = "main.db"
FILE_PATH = r"data\Ростовская область\Урожайность.csv"
INDICATOR_ID = 3


def parse_yield_csv(path: str) -> pd.DataFrame:
    rows = []
    ...
    # ищем строку с годами: первая ячейка пустая, дальше идут числа годов
    header_idx = None
    ...
    for row in rows[header_idx + 1:]:
        ...
        for year, raw_val in zip(years, values):
            ...
            if district is None:
                print(f"[SKIP] нет district для строки: {label}")
                continue

            if farm_type is None:
                print(f"[SKIP] нет farm_type для строки: {label}")
                continue
    ...
    # на всякий случай убираем точные дубли
    df = df.drop_duplicates(
        subset=["district", "farm_type", "crop", "year"],
        keep="last"
