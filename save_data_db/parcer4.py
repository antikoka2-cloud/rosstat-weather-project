...
def parse_mineral_fertilizers_csv(path: str) -> pd.DataFrame:
    ...
    for row in rows[header_idx + 1:]:
        ...
        # пропускаем служебные строки, если вдруг попадутся с мусором
        if low in {
            "муниципальный район",
            "городской округ",
        ...
        # в этом файле строка с данными уже содержит district в первой ячейке
        district_name = label
    ...
    df = df.dropna(subset=["district", "year", "value"])
